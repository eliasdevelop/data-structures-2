...
public class Escola implements Serializable {
	...
	private HashMapa<Aluno, ListaEncOrd<Disciplina>> mapAlunos;
	private ListaEncOrd<Disciplina> disciplinas;
	

	public Escola() {
		mapAlunos = new HashMapa<Aluno, ListaEncOrd<Disciplina>>();
		disciplinas = new ListaEncOrd<Disciplina>();
	}
	
	public void addAluno(Aluno aluno) throws Exception {
		if (mapAlunos.contains(aluno))
			throw new Exception("Aluno ja matriculado.");
		mapAlunos.put(aluno, new ListaEncOrd<Disciplina>());
	}

	public void removeAluno(Aluno aluno) throws Exception {
		
		ListaEncOrd<Disciplina> disciplinas = mapAlunos.getValor(aluno);
	...
	public Aluno[] getAlunos() throws Exception {
		...
		if (arrayChaveValor != null) {
			...
			for (int cont = 0; cont < alunos.length; cont++) {
				alunos[cont] = ((ChaveValor<Aluno, Disciplina>) arrayChaveValor[cont])
						.getChave();
			}
			return alunos;
		}
	...
	public Aluno getAluno(Aluno aluno) throws Exception {
		...
		if (arrayChaveValor != null) {
			...
			for (int cont = 0; cont < arrayChaveValor.length; cont++) {
				temp = ((ChaveValor<Aluno, Disciplina>) arrayChaveValor[cont])
						.getChave();
	...
	private Disciplina acharDisc(ListaEncOrd<Disciplina> lista, Disciplina disc) {
	...
	private Disciplina removerDisc(ListaEncOrd<Disciplina> lista,
	...
	private boolean discComAlunos(Disciplina disc) {
		MyIteratorMapa<Aluno, ListaEncOrd<Disciplina>> it = mapAlunos
				.iterator();
		ChaveValor<Aluno, ListaEncOrd<Disciplina>> temp = it.getFirst();
		ListaEncOrd<Disciplina> disciplinasTemp;
	...
	public void matricularAlunoDisc(int matriculaAluno, Disciplina disc)
			throws Exception {
		ListaEncOrd<Disciplina> lista = mapAlunos.getValor(new Aluno(
				matriculaAluno));
		if (lista != null) {
			Disciplina achada = acharDisc(disciplinas, disc);
			if (achada != null) {
	...
	public Disciplina[] listaDisciplinasDoAluno(Aluno aluno) throws Exception {
		ListaEncOrd<Disciplina> discAluno = mapAlunos.getValor(aluno);
		if (discAluno.size() != 0) {
	...
	public Aluno[] listaAlunosMatriculadosDisciplina(Disciplina disc)
			throws Exception {
		ListaEncOrd<Aluno> alunos = new ListaEncOrd<Aluno>();
